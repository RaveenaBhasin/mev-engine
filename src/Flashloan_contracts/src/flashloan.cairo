...
pub mod FlashLoanContract {
    ...
    impl FlashLoanImpl of IFlashloanReceiver<ContractState> {
        ...
        fn start_flashloan(ref self: ContractState, amount: u256, token_route: Array<Swap>) {
            let vesu = self.vesu_dispatcher.read();
            ...
            token_route.serialize(ref calldata);
            vesu.flash_loan(my_address, token, amount, false, calldata.span());
        }
    }
}
